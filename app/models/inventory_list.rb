...
class InventoryList < ApplicationRecord
  ...
  scope :belonging_to_user, ->(user) { joins(:playthrough).where(playthroughs: { user_id: user.id }).order('inventory_lists.updated_at DESC') }
  ...
  def format_title
    ...
    if title.blank?
      max_existing_number = playthrough.inventory_lists.where("title LIKE 'My List %'").pluck(:title).map {|t| t.gsub('My List ', '').to_i }
                              .max || 0
