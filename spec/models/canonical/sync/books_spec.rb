  ...
  describe '::perform' do
    ...
    context 'when preserve_existing_records is false' do
      ...
      context 'when there are existing book records in the database' do
        ...
        it "removes canonical ingredient associations that don't exist in the JSON data" do
          book_in_json.canonical_ingredients.create!(item_code: '12345678', name: 'Venus Fly Trap', unit_weight: 1, add_on: 'base', collectible: true, purchasable: false, rare_item: true)
    ...
    describe 'error logging' do
      ...
      context 'when an ActiveRecord::RecordInvalid error is raised' do
        let(:errored_model) { instance_double Canonical::Book, errors:, class: class_double(Canonical::Book, i18n_scope: :activerecord) }
        ...
        before do
          ...
          allow_any_instance_of(Canonical::Book).to receive(:save!).and_raise(ActiveRecord::RecordInvalid, errored_model)
        ...
        it 'logs and reraises the error', :aggregate_failures do
          ...
          expect(Rails.logger).to have_received(:error).with("Error saving canonical book \"000F5CB8\": Validation failed: Title can't be blank")
        end
      end
      ...
      context 'when another error is raised pertaining to a specific model' do
        ...
        it 'logs and reraises the error', :aggregate_failures do
          ...
          expect(Rails.logger).to have_received(:error).with('Unexpected error StandardError saving canonical book "000F5CB8": foobar')
        end
      end
