...
RSpec.describe ClothingItem, type: :model do
  describe 'validations' do
    ...
    describe '#canonical_clothing_item' do
      ...
      context 'when the canonical clothing item is not unique' do
        ...
        before { create_list(:clothing_item, 3, canonical_clothing_item:, game:) }
      ...
      context 'when the canonical clothing item is unique' do
        let(:canonical_clothing_item) { create(:canonical_clothing_item, max_quantity: 1, unique_item: true, rare_item: true) }
        ...
        context 'when there is another matching clothing item for another game' do
          before { create(:clothing_item, canonical_clothing_item:) }
        ...
        context 'when there is another matching clothing item for the same game' do
          before { create(:clothing_item, canonical_clothing_item:, game:) }
  ...
  describe '::before_validation' do
    ...
    context 'when there is a single matching canonical model' do
      let!(:matching_canonical) { create(:canonical_clothing_item, :with_enchantments, name: 'Fine Clothes', unit_weight: 1, magical_effects: 'Something') }

      let(:item) { build(:clothing_item, name: 'Fine clothes', unit_weight: 1) }

      before { create(:canonical_clothing_item, name: 'Fine Clothes', unit_weight: 2) }
    ...
    context 'when there are multiple matching canonical models' do
      let!(:matching_canonicals) { create_list(:canonical_clothing_item, 2, :with_enchantments, name: 'Fine Clothes', unit_weight: 1) }
    ...
    context 'when updating in-game item attributes' do
      ...
      context 'when the update changes the canonical association' do
        let!(:new_canonical) { create(:canonical_clothing_item, name: 'Roughspun Tunic', unit_weight: 5) }
      ...
      context 'when the update results in an ambiguous match' do
        before { create_list(:canonical_clothing_item, 2, name: 'Roughspun Tunic', unit_weight: 5) }
  ...
  describe '::after_create' do
    context 'when there is a single matching canonical model' do
      ...
      context 'when the new item has its own enchantments' do
        let(:item) { create(:clothing_item, :with_enchantments, name: 'Fine clothes', unit_weight: 1) }
    ...
    context 'when there are multiple matching canonical models' do
      let!(:matching_canonicals) { create_list(:canonical_clothing_item, 2, :with_enchantments, name: 'Fine Clothes', unit_weight: 1, magical_effects: 'Something') }
  ...
  describe '#canonical_models' do
    ...
    context 'when there are matching canonical models' do
      before { create(:canonical_clothing_item, name: 'Something Else') }
      ...
      context 'when multiple attributes have to match' do
        ...
        before { create(:canonical_clothing_item, name: item.name, unit_weight: 1) }
      ...
      context 'when there are enchantments' do
        ...
        let!(:matching_canonicals) { create_list(:canonical_clothing_item, 2, enchantable: false) }
    ...
    context "when the matching canonical model isn't the one that's assigned" do
      ...
      let!(:new_canonical) { create(:canonical_clothing_item, name: 'Roughspun Tunic', unit_weight: 1) }
  ...
  describe 'adding enchantments' do
    ...
    before { create_list(:canonical_clothing_item, 2, :with_enchantments, name: 'Foobar', enchantable:) }
    ...
    context 'when the added enchantment narrows it down to one canonical match' do
      subject(:add_enchantment) { create(:enchantables_enchantment, enchantable: item, enchantment: Canonical::ClothingItem.last.enchantments.first, strength: Canonical::ClothingItem.last.enchantments.first.strength) }
