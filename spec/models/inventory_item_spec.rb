...
RSpec.describe InventoryItem, type: :model do
  ...
  describe '::combine_or_create!' do
    context 'when there is an existing item on the same list with the same (case-insensitive) description' do
      subject(:combine_or_create) { described_class.combine_or_create!(description: 'existing item', quantity: 1, list: inventory_list, notes: notes2) }

      let!(:existing_item) { create(:inventory_item, description: 'ExIsTiNg ItEm', quantity: 2, unit_weight: 0.3, list: inventory_list, notes: notes1) }
      ...
      context 'when the new item has a unit_weight' do
        subject(:combine_or_create) { described_class.combine_or_create!(description: 'existing item', quantity: 1, list: inventory_list, unit_weight: 0.2, notes: notes2) }
      ...
      context 'when the list is an aggregate list' do
        ...
        it 'leaves the notes as nil' do
          combine_or_create
          expect(inventory_list.list_items.last.reload.notes).to be_nil
        end
      end
    end
