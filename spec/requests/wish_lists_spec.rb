...
RSpec.describe 'WishLists', type: :request do
  ...
  describe 'POST playthroughs/:playthrough_id/wish_lists' do
    subject(:create_wish_list) { post "/playthroughs/#{playthrough.id}/wish_lists", params: { wish_list: {} }.to_json, headers: }
    ...
    context 'when authenticated' do
      ...
      context 'when all goes well' do
        ...
        context 'when an aggregate list has also been created' do
          it 'creates a new wish list' do
            expect { create_wish_list }
              .to change(playthrough.wish_lists, :count).from(0).to(2) # because of the aggregate list
          end

          it 'returns both wish lists' do
            create_wish_list
            expect(response.body).to eq(playthrough.wish_lists.index_order.to_json)
          end
        ...
        context 'when only the new wish list has been created' do
          ...
          it 'returns only the created list' do
            create_wish_list
            expect(response.body).to eq([playthrough.wish_lists.unscoped.last].to_json)
          end
        ...
        context 'when the request does not include a body' do
          subject(:create_wish_list) { post "/playthroughs/#{playthrough.id}/wish_lists", headers: }
          ...
          it 'creates the wish list with a default title' do
            create_wish_list
            expect(playthrough.wish_lists.last.title).to eq('My List 1')
          end
        end
      end
      ...
      context 'when the params are invalid' do
        subject(:create_wish_list) do
          post "/playthroughs/#{playthrough.id}/wish_lists",
               params: {
        ...
        let(:playthrough) { create(:playthrough, user:) }
        let(:existing_list) { create(:wish_list, playthrough:) }
  ...
  describe 'GET playthroughs/:playthrough_id/wish_lists' do
    subject(:get_index) { get "/playthroughs/#{playthrough.id}/wish_lists", headers: }
  ...
  describe 'DELETE /wish_lists/:id' do
    ...
    context 'when authenticated' do
      ...
      context 'when the wish list exists' do
        ...
        context "when this is the playthrough's last regular wish list" do
          let!(:aggregate_list_id) { playthrough.aggregate_wish_list.id }
        ...
        context "when this is not the playthrough's last regular wish list" do
          let(:expected_response_body) do
            {
